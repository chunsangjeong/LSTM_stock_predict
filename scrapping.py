...
def stock_price_pages_to_df(code, days_limit=30):
    ...
    filename = get_data_filename(code)
...
def find_stock_index(company_name='삼성'):
    try:
...
def get_data_filename(stock_code):
    saved_dir = 'data'
    filename = 'price_'
...
def generate_report(company_name='삼성'):
    stock_code = find_stock_index(company_name)
    filename = get_data_filename(stock_code)
    stock_price_pages_to_df(stock_code, 100)
    #show_graph(filename)
